...
import type { Profile, Product, DbStatus, TelegramLog, ProjectLog, WorkerStatus, ProductView, Campaign, CampaignTemplate, CustomLink, BackgroundExportStatus, TelegramStats, BotUser, RpsDataPoint, GitHubSyncProfile } from "@/types";
...
export default function FunPayWorkerDashboard() {
  ...
  const [githubSyncProfiles, setGithubSyncProfiles] = useState<GitHubSyncProfile[]>([]);
  const [isSyncing, setIsSyncing] = useState<string | boolean>(false);
  ...
  const fetchConfig = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        setGithubSyncProfiles(data.githubSyncProfiles || []);
      } else {
  ...
  const handleSyncWithGitHub = async (profilesToSync: {repoUrl: string, token: string}[]) => {
    const syncAll = profilesToSync.length > 1;
    const profileId = syncAll ? true : (profilesToSync[0] ? githubSyncProfiles.find(p => p.repoUrl === profilesToSync[0].repoUrl)?.id : false);
    ...
    try {
        const result = await syncWithGitHub({ profiles: profilesToSync });
  ...
  const handleSaveGitHubProfile = (profile: GitHubSyncProfile) => {
    const newProfiles = [...githubSyncProfiles];
    if(profile.id) { // Editing
      const index = newProfiles.findIndex(p => p.id === profile.id);
      if(index !== -1) {
        newProfiles[index] = profile;
      }
    } else { // Creating
      newProfiles.push({ ...profile, id: Date.now().toString() });
    }
    setGithubSyncProfiles(newProfiles);
    handleSaveConfig({ githubSyncProfiles: newProfiles });
  };

  const handleDeleteGitHubProfile = (profileId: string) => {
    const newProfiles = githubSyncProfiles.filter(p => p.id !== profileId);
    setGithubSyncProfiles(newProfiles);
    handleSaveConfig({ githubSyncProfiles: newProfiles });
    toast({ title: 'Профиль удален', variant: 'destructive' });
  }
    ...
    case "telegram-panel":
      ...
      case "settings":
        return <SystemSettingsTab
          ...
          githubSyncProfiles={githubSyncProfiles}
          handleSaveGitHubProfile={handleSaveGitHubProfile}
          handleDeleteGitHubProfile={handleDeleteGitHubProfile}
